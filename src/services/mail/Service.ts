...
export default class MailServer extends BaseService {
    ...
    public async sendMail(playerMail): Promise<any> {
        ...
        if (res === 0) {
            throw new Exceptions(8076, 'mail sent error');
        } else {
            return res;
        }
        // if (res === 0) {
    ...
    // 修改信箱
    public async modifyEmail(memberNo, email, verificationCode, password): Promise<any> {
        // 驗證信箱
        const res = await this.repository.verification(email, verificationCode);
        if (res === 0) {
            throw new Exceptions(8002, 'mail verification error');
        } else {
            const result = await this.repository.modifyEmail(memberNo, email, password);  // 驗證成功才執行修改信箱
            if (result !== 'success') {
                throw new Exceptions(result, 'API server error');
            } else {
                return result;
            }
        }
