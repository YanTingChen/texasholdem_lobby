...
export default class RegisterServer extends BaseService {
    ...
    public async register(account, verificationCode, password, platform, device, ip, browser): Promise<any> {
        const verify = await this.repository.verification(account, verificationCode);
        if (verify === 0) {
