...
export default class LoginController extends BaseController {
    ...
    public async visitor(ctx: IContext) {
        ...
        const device = ctx.request.body.reqData.device;
        const browser = ctx.request.body.reqData.browser;
        ctx.body = new BaseResponse(await this.service.visitorLoginCheck(ip, platform, device, browser));
    }

    // 快速登入(自動登入)
    @httpPost('/quick')
    public async quick(ctx: IContext) {
        const quickPlayer = verify(ctx.request.body.reqData.token, config.jwt.privateKey);
        const no = _.toNumber(Utils.Decryption_AES_ECB_128(JSON.parse(JSON.stringify(quickPlayer)).id));
        const address = {ip: ''};
        if (ctx.request.ip !== '::ffff:127.0.0.1') {
            address.ip = ctx.request.ip.match(/\d+.\d+.\d+.\d+/)[0];
        }
        ...
        const device = ctx.request.body.reqData.device;
        const browser = ctx.request.body.reqData.browser;
        ctx.body = new BaseResponse(await this.service.quickLoginCheck(no, ip, platform, device, browser));
    }
}
