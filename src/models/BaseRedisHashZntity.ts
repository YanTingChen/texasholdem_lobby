...
export default abstract class BaseRedisHashZntity {
    ...
    public async execDeleteAll(id: number) {
        if (!_.isUndefined(id)) {
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, this.constructor.name + 'id is undefined');
        } else {
    ...
    public async execReuseRedis(id: number, source: any) {
        if (!_.isUndefined(source)) {
            ...
            throw new LibsExceptions(ErrorStatusCode.STATUS_FAIL, this.constructor.name + 'source is undefined');
        }
    }
}
